...
import com.benchpress200.photique.singlework.presentation.request.SearchSingleWorksRequest;
import com.benchpress200.photique.singlework.presentation.response.SearchSingleWorksResponse;
...
public class SingleWorkQueryController {
    ...
    @GetMapping
    public ResponseEntity<?> searchSingleWorks(
            @ModelAttribute @Valid SearchSingleWorksRequest searchSingleWorksRequest
    ) {
        SearchSingleWorksQuery searchSingleWorksQuery = searchSingleWorksRequest.toQuery();
        SearchSingleWorkResult searchSingleWorkResult = singleWorkQueryService.searchSingleWorks(
                searchSingleWorksQuery);
        SearchSingleWorksResponse searchSingleWorksResponse = SearchSingleWorksResponse.from(searchSingleWorkResult);

        return ResponseHandler.handleResponse(
                HttpStatus.OK,
                SingleWorkResponseMessage.WORK_SEARCH_SUCCESS,
