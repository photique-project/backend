package com.benchpress200.photique.singlework.presentation.command.controller;
...
import com.benchpress200.photique.singlework.presentation.command.constant.SingleWorkCommandResponseMessage;
import com.benchpress200.photique.singlework.presentation.command.dto.request.SingleWorkCreateRequest;
import com.benchpress200.photique.singlework.presentation.command.dto.request.SingleWorkUpdateRequest;
...
public class SingleWorkCommandController {
    ...
    public ResponseEntity<?> postSingleWork(
            @RequestPart(MultipartKey.SINGLEWORK) @Valid SingleWorkCreateRequest singleWorkCreateRequest,
            @RequestPart(MultipartKey.IMAGE) MultipartFile image
    ) {
        SingleWorkCreateCommand singleWorkCreateCommand = singleWorkCreateRequest.toCommand(image);
        singleWorkCommandService.postSingleWork(singleWorkCreateCommand);
    ...
    public ResponseEntity<?> updateSingleWorkDetails(
            ...
            @RequestBody SingleWorkUpdateRequest singleWorkUpdateRequest
    ) {
        SingleWorkUpdateCommand singleWorkUpdateCommand = singleWorkUpdateRequest.toCommand(singleWorkId);
        singleWorkCommandService.updateSingleWorkDetails(singleWorkUpdateCommand);
