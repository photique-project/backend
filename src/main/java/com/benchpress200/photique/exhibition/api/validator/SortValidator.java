package com.benchpress200.photique.exhibition.api.validator;
...
public class SortValidator {
    ...
    private static final Set<String> EXHIBITION_SEARCH_ALLOWED_FIELDS =
            Set.of("createdAt", "likeCount", "viewCount");

    private static final Set<String> MY_EXHIBITION_SEARCH_ALLOWED_FIELDS =
            Set.of("createdAt");
    ...
    public static Sort verifyAndParseExhibitionSearch(String sortValue) {
        ...
        // 필드 검증
        if (!EXHIBITION_SEARCH_ALLOWED_FIELDS.contains(field)) {
            throw new InvalidFieldToSearch(INVALID_SORT);
        }
    ...
    public static Sort verifyAndParseMyExhibitionSearch(String sortValue) {
        // null 이거나 빈 값이라면 기본 정렬 기준으로
        if (sortValue == null || sortValue.isBlank()) {
            return Sort.by(DEFAULT_DIRECTION, DEFAULT_FIELD);
        }
        ...
        // 필드 검증
        if (!MY_EXHIBITION_SEARCH_ALLOWED_FIELDS.contains(field)) {
            throw new InvalidFieldToSearch(INVALID_SORT);
        }
